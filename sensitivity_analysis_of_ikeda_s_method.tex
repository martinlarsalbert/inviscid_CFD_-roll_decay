...
the attention. Ikeda has in a series of papers proposed
...
or a more advanced potential flow method such as FNPF as is done in this study.

Ikeda produced many papers about various aspects of roll damping where
most of them are translations from original manuscripts written in Japanese.
...
study. This is therefore the only component in Ikeda's method that needs
...
values for \(\kappa\) from Ikeda's paper.
...
an asterisk, or star symbol (*). The reason seems to be that Ikeda wanted to signal that
...
\end{equation}
    ...
    Instead of trying to invent a complicated mathematical expression
...
\(C_r\) was predicted with Ikeda's method and the alternative decision tree model
...
\[ \Delta_n = \frac{\phi_{a,n}}{\phi_{a,n+2}}\]
    ...
    The nondimensional damping coefficient for each peak \(\zeta_n\) can be
...
implicitly with FNPF as described in the next section is used together with the
viscous damping contributions from Ikeda's method. The wave damping was
...
The method used in this study employs a boundary element method (BEM) (ref Hess and Smith) to solve the boundary value problem for the velocity potential.

The free surface boundary conditions and the motions of the floating body introduce time dependency to the boundary value problem. The BEM is coupled with the mixed Eulerian-Lagrangian method (MEL) (ref Longuet-Higgins and Cokelet 1976) which is used for the evolution of free surface. A fourth-order Adams-Bashforth-Moulton time integral scheme is then used to evolve free surface and the rigid-body body motions in time.
