...
oscillations. The ship loses energy over time due to the damping which
...
identidy the damping from these tests are used: the PIT method,
described in the next section and the logartithmic decrement described
in section: "\nameref{se:time_and_frequency}".
...
a linear model (see eq.\ref{eq:linear}). For this case the
relation between $\zeta$ and $B_1$ can be expressed as:
