...
\quad     A common way to determine the roll damping of a ship is to conduct a
...
\quad     Time traces of the roll motion from roll decay tests in model scale
...
\quad     A parameters identification technique (PIT) similar to
...
\quad     Where $B_{44}(\dot{\phi})$ and $C_{44}(\phi)$ are the damping and
...
\quad     The total equation is then written:
...
\quad     This mathematical model can be reduced to a quadratic damping model when
...
\quad     The parameters of this equation can be identified using least square fit
...
\quad     This numerical solution can be compared with an analytical solution for
...
\quad     and the natural frequency can be obtained from:
...
\quad     The analytical and numerical solutions are very similar according to the
