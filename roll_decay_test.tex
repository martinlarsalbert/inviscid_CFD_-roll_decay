...
example: $A_{44} = 1.0$, $B_1 = 0.3$, $C_1 = 5.0$ (with 0.01 s
time step) shown in Fig.\ref{fig:analytical_numerical}.
\begin{figure}[H]
...
standard deviation has been added to the simulated roll signal in
...
parameters to converge. The Differentiation approach has therefore been
used as a pre-step to obtain a very good first guess of the parameters
that can be passed on to the Integration approach. This has been used
for both signals from FNPF and model tests, where in the latter case
