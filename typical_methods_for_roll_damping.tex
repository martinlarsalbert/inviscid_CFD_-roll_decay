...
can be seen in this graph:
    ...
    Time traces of the roll motion from roll decay tests in model scale experiments as well as from FNPF
...
determined in order to get the best fit to a roll decay time signal. A derivation
...
using numerical differentiation of a low-pass filtered roll signal or Kalman
filtered roll signal. The filtering will however introduce some errors
in itself. So instead of using this "Differentiation approach", it has been
...
guess of the parameters. The Differentiation approach has therefore been used
